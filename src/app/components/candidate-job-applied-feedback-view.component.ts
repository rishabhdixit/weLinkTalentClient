...
@Component({
	selector: 'app-candidate-job-applied-feedback-view',
	template: `
		<div class="row">
...
export class CandidateJobAppliedFeedbackViewComponent implements OnInit {
	...
	private constructFeedBacks() {
		for (let id of this.getFeedBackKeys()) {
			this.jobAppliedFeedBacks.push(this.candidateJobApplied.feedback[id].feedback as RefereeFeedback);
		}
	}

	private getFeedBackKeys() {
		if (!this.candidateJobApplied) {
			return [];
		}
		return (this.candidateJobApplied.feedback ? Object.keys(this.candidateJobApplied.feedback) : []);
	}
}
